...
class BindingValidator(BaseValidator):
        ...
        for index, binding in enumerate(bindings):
            ...
            if capability is None:
                reporter.add(
                    ...
                    message="本地静态 schema 未包含该 capabilityId，当前上下文无法确认其参数和输出路径。",
                    fix_hint="提供完整 artifact/effectiveCapabilities 或对应 capability schema 后再裁决；不要据此断言设备不支持。",
                )
                continue
    ...
    def _check_event_handlers(self, context, rules, reporter) -> None:
        ...
        for component in context.components:
            ...
            for index, handler in enumerate(handlers):
                ...
                if call in {"clickToApi", "clickToIntent"}:
                    self._check_intent(pointer, args, spec, reporter)
                elif call == "clickToDeeplink":
                    self._check_deeplink(pointer, args, spec, reporter)
    ...
    def _check_deeplink(self, pointer: str, args: dict[str, Any], spec: dict[str, Any], reporter) -> None:
        intent = args.get("intentName")
        supported = spec.get("supportedIntents", {})
        if intent not in supported:
            reporter.add(
                ...
                message="clickToDeeplink.intentName 不在支持列表中。",
            )
            return
        target = supported[intent]
        for field in ("bundleName", "abilityName"):
            if args.get(field) != target.get(field):
                reporter.add(
                    "error",
                    "EVENT_ARGUMENT_INVALID",
                    "semantic",
                    "genui",
                    line=2,
                    json_pointer=f"{pointer}/args/{field}",
                    actual=args.get(field),
                    expected=target.get(field),
                    message="deeplink 目标字段必须与声明能力完全一致。",
                )
        if args.get("uri") not in target.get("uris", []):
            reporter.add(
                "error",
                "EVENT_ARGUMENT_INVALID",
                "semantic",
                "genui",
                line=2,
                json_pointer=f"{pointer}/args/uri",
                actual=args.get("uri"),
                expected=target.get("uris", []),
                message="deeplink uri 必须来自目标能力声明的页面列表。",
            )
