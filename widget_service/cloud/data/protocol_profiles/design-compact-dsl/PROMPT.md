...
**格式化主读数例外（与转换前校验一致）**：适用于明确描述为温度、时长或百分比的主读数，不限于单一 data 根；名称、日期、时间、状态不适用。内容可以是直接 PathBinding，也可以是只引用一个字段并追加真实单位的简单 Expression，不得拼接名称、状态或说明。Text 必须单行、无 padding/margin、显式高度至少为字号的 1.4 倍，并与实际主内容槽同宽：2x2 使用 136vp，2x4 全宽使用 296vp，W9/W10 左侧大背板等 144×136vp 区域使用内部 120vp；父级 Column/Row 必须同宽且无 padding。只允许 20fp 或 24fp；温度优先 24fp，时长优先 20fp，须以较长合法值（负温、100%、两位数小时和分钟）加 20% 余量检查宽度。W8、小内容蒙版与环中心不使用该例外；不能通过增大 Text 声明宽度突破父级预算。
