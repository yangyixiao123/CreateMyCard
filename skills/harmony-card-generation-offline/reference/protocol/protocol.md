...
- `createSurface` 默认只写 `surfaceId`、`catalogId`、`width`、`height`；`width/height` 必须写 CardSpec/profile 对应的数值尺寸。新卡片不要为了同步 root 圆角而写 `createSurface.styles`。若宿主明确需要外层形状和裁切控制，`createSurface.styles` 只允许 `borderRadius`、`clip`；不支持 `theme`。
- `updateComponents` 必须在 `createSurface` 之后，同一 surface 仅发送一次完整组件树。
- `updateComponents.root` 必须引用 `components` 中存在的组件 id。
- root 组件是唯一卡片 shell，承载 `width`、`height`、`padding`、`borderRadius`、`clip` 和 `backgroundColor` / `linearGradient` / `backgroundImage` 等布局和表面样式；root `width/height` 必须写 CardSpec/profile 对应的数值尺寸。背景也可由 root 下的真实背景组件承载。
