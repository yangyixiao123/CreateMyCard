...
- 要求直接输出 DSL/CardSpec；本 skill 默认走云侧生成链路，不在正常链路直接输出 DSL/CardSpec。只有 `generateWidgetCard` 不可用、调用失败或结果不符合预期时，才允许进入兜底生成。
...
生成 `generateWidgetCard` 入参中的 `title` 和 `description` 时遵守：

- `title` 是展示给用户的静态短标题，尽量不超过 8 个字。
- `description` 是展示给用户的静态短概述，尽量不超过 12 个字。
- 两者只表达卡片用途或核心场景，例如“通勤日常”“天气日程速览”。
- 不使用动态绑定、变量、时间表达式、能力 ID、包名、内部字段名或错误码。
- 不把 `title` 和 `description` 当成最终 CardSpec；微服务会结合能力过滤和降级结果规范化后写入最终 CardSpec。
- 如果用户标题过长，提炼为短标题；如果用户没有明确标题，按核心场景生成中性短标题。
...
- 如果所有核心能力都缺失且没有入口或静态价值，不要自行输出 `genWidgetResult`；调用微服务后按其 `unsupported` 回复，或在工具不可用时进入兜底链路并说明无法生成动态卡片的原因。
