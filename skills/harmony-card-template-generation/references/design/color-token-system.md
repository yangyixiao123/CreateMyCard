...
- 背景颜色的**放置位置**不在本文范围：背景字段必须落在 `root.styles`（或 root 下真实背景组件），不得放 `createSurface`，见 [`../generation-contract.md`](../generation-contract.md) 与 [`../protocol/protocol.md`](../protocol/protocol.md)。

## 配色决策流程

颜色选择先做场景判断，再做 token 映射；不要从“好看、清新、高级、科技感”这类抽象词直接挑色。

1. 从 `../generation-contract.md` 提取服务对象、主问题、内容密度、关键事实和层级。
2. 命名 2-4 个具体锚点，例如天空层、雨玻璃、日历纸、跑道、票纸、电池环、包裹标签、药盒、收据纸、夜间舞台。
3. 按 `../routing-and-style.md` 选定模板和风格档案，再决定一个主场景色族。
