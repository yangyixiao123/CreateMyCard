...
- root 组件是唯一卡片 shell，承载 `width`、`height`、`padding`、`borderRadius`、`clip` 和 `backgroundColor` / `linearGradient` / `backgroundImage` 等布局和表面样式；root `width/height` 必须写 `"matchParent"`，无论 `2x2` 还是 `2x4`，root `borderRadius` 固定为 `18` 且 `clip` 固定为 `true`；内部组件按 `2x2 = 160vp x 160vp`、`2x4 = 320vp x 160vp` 的实际预算落数值。背景也可由 root 下的真实背景组件承载。
